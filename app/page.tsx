...
export default function Page() {
    ...
    setSessions((prev) =>
      prev.map((session) =>
        session.id === activeSessionId
          ? {
              ...session,
              chatHistory: [...session.chatHistory, { role: 'user', text: input }],
            }
          : session
      )
    );
  ...
  return (
    <main className="flex min-h-screen bg-white">
      {/* Left side with your previous CSS */}
  ...
  className="max-w-full max-h-[60vh] object-contain rounded border shadow-sm mx-auto"
      ...
      {/* Right side chat sessions panel */}
     <section className="md:w-1/2 flex flex-col p-6 max-h-screen bg-gray-50">
  ...
  <div className="flex-grow overflow-y-auto space-y-6">
    ...
    {sessions.map((session) => (
      ...
      >
        ...
        <pre className="whitespace-pre-wrap text-gray-700 text-sm mb-4">{session.extractedText}</pre>
